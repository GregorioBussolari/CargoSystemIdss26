...
public class MultipleUserButtonPressedClient {
    ...
    protected void test1() throws Exception {
        ...
        CommUtils.outblue(name + " | test1 sent FIRST buttonPressed to ioport");
        
        // 2. Attesa
        int delayTime = 4000;
        CommUtils.outblue(name + " | waiting for " + delayTime + " ms before second press...");
        CommUtils.delay(delayTime);
        
        // 3. Premo secondo pulsante
        conn.forward(buttonPressed2);
        CommUtils.outblue(name + " | test1 sent SECOND buttonPressed to ioport");
    }

    public static void main(String[] args) {
        try {
            new MultipleUserButtonPressedClient().doJob();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
